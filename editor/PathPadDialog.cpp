...
CPathPadDialog::CPathPadDialog(CWnd *pParent /*=NULL*/) : CKeypadDialog(CPathPadDialog::IDD, pParent) {
...
void CPathPadDialog::ResetNodesForComboBox(int handle, char selection_type) {
  ...
  if (Viewer_object->roomnum >= 0 && Viewer_object->roomnum <= Highest_room_index &&
      Rooms[Viewer_object->roomnum].used) {
    if (selection_type == EBVIEW_ROOM || selection_type == EBVIEW_ROOM_AND_NEXT_ROOMS) {
      ...
      if (selection_type == EBVIEW_ROOM_AND_NEXT_ROOMS) {
        ...
        for (i = 0; i < Rooms[Viewer_object->roomnum].num_portals; i++) {
          ...
          if (!f_found && Rooms[Viewer_object->roomnum].portals[i].croom >= 0 &&
    ...
    if (selection_type == EBVIEW_ROOM || selection_type == EBVIEW_ROOM_AND_NEXT_ROOMS) {
      ...
      if (selection_type == EBVIEW_ROOM_AND_NEXT_ROOMS) {
        ...
        for (i = 0; i < BOA_num_connect[tr]; i++) {
          ...
          if (!f_found && BOA_connect[tr][i].roomnum >= 0 && !(Rooms[BOA_connect[tr][i].roomnum].flags & RF_EXTERNAL)) {
...
void CPathPadDialog::OnSetfocusAinCurrentNode() { ResetNodesForComboBox(IDC_AIN_CURRENT_NODE, ENode_view_type); }

void CPathPadDialog::OnSetfocusAinOtherNode() { ResetNodesForComboBox(IDC_AIN_OTHER_NODE, ENode_view_type); }
