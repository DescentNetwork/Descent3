...
posix_stream_base::posix_stream_base(void* memory, size_t length, std::ios_base::openmode mode)
{
...
bool posix_stream_base::open(const std::filesystem::path& path, std::ios_base::openmode mode)
{
  if(is_open())
  {
...
bool posix_stream_base::eof(void) const
{
  return feof(m_fp);
}
