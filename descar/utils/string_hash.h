...
constexpr uint64_t fnv1_64(string_type data) {
...
static inline uint64_t hash(const char *str, std::size_t sz) noexcept
  { return fnv1_64<std::string>(std::move(std::string(str, sz))); }

static inline uint64_t hash(const char16_t *str, std::size_t sz) noexcept
  { return fnv1_64<std::u16string>(std::move(std::u16string(str, sz))); }
