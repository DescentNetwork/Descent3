...
struct fixed_string_t : std::array<char, length>
{
  ...
  inline std::string string(void) const
    { return std::string(std::begin(*this), std::find(std::begin(*this), std::end(*this), '\0')); }
...
template<std::size_t length>
constexpr byte_ostream& operator <<(byte_ostream& output, const fixed_string_t<length>& fs)
{
...
template<std::size_t length>
constexpr byte_istream& operator >>(byte_istream& input, fixed_string_t<length>& fs)
{
