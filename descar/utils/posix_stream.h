...
public:
  ...
  posix_stream_base(const std::filesystem::path& path, std::ios_base::openmode mode) { open(path, mode); }
...
struct posix_istream : posix_stream_base,
                      byte_istream
{
  ...
  byte_istream& operator >>( int8_t& data) { data = get(); return *this; }
  byte_istream& operator >>(uint8_t& data) { data = get(); return *this; }
};

struct posix_ostream : posix_stream_base,
                      byte_ostream
{
  using posix_stream_base::posix_stream_base;
  using posix_stream_base::operator=;
