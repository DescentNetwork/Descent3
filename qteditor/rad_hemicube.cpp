...
void ScanRadiosityPoly(g3Point **pl, int nv, int element_id) {
  ...
  auto leftedge = [&left_height, &cp, &Left_x, &Left_z, &Delta_left_x, &Delta_left_z, &next_break_left, &vlb, &vlt]()
  {
  ...
  auto rightedge = [&right_height, &cp, &Right_x, &Right_z, &Delta_right_x, &Delta_right_z, &next_break_right, &vrb, &vrt]()
  {
