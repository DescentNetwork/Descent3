...
// #include <SDL3/SDL_assert.h> // Removed for standalone build
...
#define DEBUG_BREAK()                                                                                                  \
  do {                                                                                                                 \
    ...
    // SDL_assert(false); // Removed for standalone build                                                                                                 \
    if (DebugBreak_callback_resume)                                                                                    \
      (*DebugBreak_callback_resume)();                                                                                 \
  } while (0)

// Does a DEBUG_BREAK() if debugging is turned on.
// Replaced with Q_ASSERT(false) for Qt compatibility
#define Int3() Q_ASSERT(false)

// Q_ASSERT is now provided by Qt - do not define it here to avoid conflicts
// Use Qt's Q_ASSERT from <QtGlobal> instead
