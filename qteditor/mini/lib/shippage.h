...
struct mngs_ship_page {
  ...
  std::string dying_image_name;
  std::string weapon_name[MAX_PLAYER_WEAPONS][MAX_WB_GUNPOINTS];
...
int mng_FindSpecificShipPage(const std::string &name, mngs_ship_page *shippage, int offset = 0);
...
int mng_SetAndLoadShip(mngs_ship_page *shippage, struct CFILE* infile);

// Given a shippage and a ship handle, attempts to make ship n correspond to
// to the shippage.
// Returns 1 on success, 0 otherwise
int mng_AssignShipPageToShip(mngs_ship_page *shippage, int n, struct CFILE* infile = NULL);
...
void mng_LoadLocalShipPage(struct CFILE* );

// Reads in a page off the net
void mng_LoadNetShipPage(struct CFILE* , bool overlay = false);
