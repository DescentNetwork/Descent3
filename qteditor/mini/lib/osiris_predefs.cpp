...
void osipf_RoomValue(int roomnum, char op, char vtype, void *ptr, int index) {
  ...
  case RMSV_V_PATH_PNT:
    if (op == VF_SET)
      rp->path_pnt = *(vector3 *)ptr;
    else if (op == VF_GET)
      *(vector3 *)ptr = rp->path_pnt;
    break;
  ...
  case RMSV_V_FACE_NORMAL:
    if (op == VF_GET)
      *(vector3 *)ptr = rp->faces[index].normal;
    break;
      ...
      if (index < 0 || index >= rp->num_faces || rp->faces[index].num_verts <= 0) {
        ...
        *(vector3 *)ptr = vector3{};
        ...
        for (i = 0; i < rp->faces[index].num_verts; i++) {
          *(vector3 *)ptr += rp->verts[rp->faces[index].face_verts[i]];
        }

        *(vector3 *)ptr /= rp->faces[index].num_verts;
      }
    }
    break;
  ...
  case RMSV_V_PORTAL_PATH_PNT:
    if (op == VF_SET)
      rp->portals[index].path_pnt = *(vector3 *)ptr; // chrishack -- make sure save game accounts for this
    else if (op == VF_GET)
      *(vector3 *)ptr = rp->portals[index].path_pnt;
    break;
...
void osipf_AIValue(int objhandle, char op, char vtype, void *ptr) {
  ...
  case AIV_V_VEC_TO_TARGET:
    if (op == VF_SET) {
      ai_info->vec_to_target_actual = ai_info->vec_to_target_perceived = *((vector3 *)ptr);
    } else {
      *((vector3 *)ptr) = ai_info->vec_to_target_perceived;
    }
    break;
  ...
  case AIV_V_LAST_SEE_TARGET_POS:
    if (op == VF_SET) {
      ai_info->last_see_target_pos = *((vector3 *)ptr);
    } else {
      *((vector3 *)ptr) = ai_info->last_see_target_pos;
    }
    break;
  ...
  case AIV_V_MOVEMENT_DIR:
    if (op == VF_SET) {
      ai_info->movement_dir = *((vector3 *)ptr);
    } else {
      *((vector3 *)ptr) = ai_info->movement_dir;
    }
    break;
  case AIV_V_ROT_THRUST_VECTOR:
    if (op == VF_SET) {
      ai_info->rot_thrust_vector = *((vector3 *)ptr);
    } else {
      *((vector3 *)ptr) = ai_info->rot_thrust_vector;
    }
    break;
  case AIV_F_FOV:
    if (op == VF_SET) {
...
void osipf_ObjectValue(int handle, char op, char var_handle, void *ptr, int index) {
  ...
  case OBJV_V_VELOCITY:
    if (op == VF_SET)
      obj->mtype.phys_info.velocity = *(vector3 *)ptr;
    else if (op == VF_GET)
      *(vector3 *)ptr = obj->mtype.phys_info.velocity;
    break;
  case OBJV_V_ROTVELOCITY:
    if (op == VF_SET)
      obj->mtype.phys_info.rotvel = *(vector3 *)ptr;
    else if (op == VF_GET)
      *(vector3 *)ptr = obj->mtype.phys_info.rotvel;
    break;
    break;
  case OBJV_V_THRUST:
    if (op == VF_SET)
      obj->mtype.phys_info.thrust = *(vector3 *)ptr;
    else if (op == VF_GET)
      *(vector3 *)ptr = obj->mtype.phys_info.thrust;
    break;
    break;
  case OBJV_V_ROTTHRUST:
    if (op == VF_SET)
      obj->mtype.phys_info.rotthrust = *(vector3 *)ptr;
    else if (op == VF_GET)
      (*(vector3 *)ptr) = obj->mtype.phys_info.rotthrust;
    break;
    break;
  case OBJV_I_FLAGS: {
...
uint8_t osipf_AITurnTowardsVectors(int objhandle, vector3 *fvec, vector3 *uvec) {
  object *objp = ObjGet(objhandle);
...
int osipf_AIGoalAdd(int objhandle, int goal_type, int level, float influence, int guid, int flags, ...) {
  ...
  case AIG_GET_TO_POS: {
    // Pop Goal Local Variables
    va_start(marker, flags);
    vector3 *pos = va_arg(marker, vector3 *);
  ...
  case AIG_FACE_DIR: // MT_INDEX
  {
    // Pop Goal Local Variables
    va_start(marker, flags);
    vector3 *v_value = va_arg(marker, vector3 *);
    va_end(marker);
  ...
  default:
    LOG_ERROR("AIGoalAdd Error: Get chris - goal %d is not available of scripting", goal_type);
  }
...
#endif
#if 0
vector3 osipf_AIGetRoomPathPoint(int roomnum) {
...
void osipf_GetGunPos(int objhandle, int gun_number, vector3 *gun_pnt, vector3 *gun_normal) {
  object *obj = ObjGet(objhandle);
...
#endif
#if 0
void osipf_GetGroundPos(int objhandle, int ground_number, vector3 *ground_pnt, vector3 *ground_normal) {
  object *obj = ObjGet(objhandle);
...
void osipf_MissionFlagSet(int flag, uint8_t value) {
  if (flag < 1 && flag > 32) {
    LOG_ERROR("Invalid flag passed to osipf_MissionFlagSet(%d)", flag);
    return;
  }
...
int osipf_MissionFlagGet(int flag) {
  if (flag < 1 && flag > 32) {
    LOG_ERROR("Invalid flag passed to osipf_MissionFlagGet(%d)\n", flag);
    return 0;
  }
...
int osipf_SoundFindId(const std::string &s_name) { return FindSoundName(IGNORE_TABLE(s_name)); }
...
void osipf_AIGoalValue(int obj_handle, int8_t g_index, char op, char vtype, void *ptr, int8_t index) {
  ...
  case AIGV_V_VEC: {
    if (op == VF_GET)
      *(vector3 *)ptr = g_ptr->g_info.vec;
    else if (op == VF_SET)
      g_ptr->g_info.vec = *(vector3 *)ptr;
  } break;
  case AIGV_V_POS: {
    if (op == VF_GET)
      *(vector3 *)ptr = g_ptr->g_info.pos;
    else if (op == VF_SET)
      g_ptr->g_info.pos = *(vector3 *)ptr;
  } break;
  ...
  case AIGV_V_VEC_TO_TARGET: {
    if (op == VF_GET)
      *(vector3 *)ptr = g_ptr->vec_to_target;
    else if (op == VF_SET)
      g_ptr->vec_to_target = *(vector3 *)ptr;
  } break;
  ...
  case AIGV_V_LAST_SEE_TARGET_POS: {
    if (op == VF_GET)
      *(vector3 *)ptr = g_ptr->last_see_target_pos;
    else if (op == VF_SET)
      g_ptr->last_see_target_pos = *(vector3 *)ptr;
  } break;
  ...
  case AIGV_V_ORIENT_FVEC: {
    if (op == VF_GET)
      *(vector3 *)ptr = g_ptr->set_fvec;
    else if (op == VF_SET)
      g_ptr->set_fvec = *(vector3 *)ptr;
  } break;
  case AIGV_V_ORIENT_UVEC: {
    if (op == VF_GET)
      *(vector3 *)ptr = g_ptr->set_uvec;
    else if (op == VF_SET)
      g_ptr->set_uvec = *(vector3 *)ptr;
  } break;
...
void osipf_SetPlayerControlMode(int pnum, bool set_to_ai) {
  Q_ASSERT(pnum >= 0 && pnum < MAX_PLAYERS);
...
void osipf_PathValue(int path_id, int node_id, char op, int changes, void *ptr) {
  ...
  if (op == VF_GET) {
    if (changes & PV_ALL) {
      ...
      changes &= ~PV_ALL;
      Q_ASSERT(changes == 0); // no other changes allowed!!!!
      return;
    }
