...
int mng_FindSpecificWeaponPage(const std::string &name, mngs_weapon_page *weaponpage, int offset = 0);
int mng_FindSpecificWeaponPage(const std::string &name, mngs_weapon_page *weaponpage);
...
int mng_SetAndLoadWeapon(mngs_weapon_page *weaponpage, struct CFILE* infile);

// Given a weaponpage and a weapon handle, attempts to make weapon n correspond to
// to the weaponpage.
// Returns 1 on success, 0 otherwise
int mng_AssignWeaponPageToWeapon(mngs_weapon_page *weaponpage, int n, struct CFILE* infile = NULL);
...
void mng_LoadLocalWeaponPage(struct CFILE* );
