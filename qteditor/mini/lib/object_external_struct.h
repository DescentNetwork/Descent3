...
struct line_info_s {
  vector3 end_pos; // start pos is the object's .pos field
};
...
struct laser_info_s {
  ...
  vector3 hit_pnt;
  vector3 hit_wall_pnt;
  vector3 hit_wall_normal;
  int32_t hit_room;
  ...
  vector3 last_smoke_pos; // Last place smoke was dropped from this weapon
...
struct physics_info {
  vector3 velocity;    // Velocity vector of this object
  vector3 thrust;      // Constant force applied to this object
  union {
    vector3 rotvel;    // Rotational velecity (angles)
    float turn_rate;
  };
  union {
  ...
  vector3 dest_pos; // destination position for interpolating velocity (for multiplayer only)
...
struct object {
  ...
  vector3 wall_sphere_offset;
  vector3 anim_sphere_offset;
