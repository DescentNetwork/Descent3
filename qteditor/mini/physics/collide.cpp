...
static void check_for_special_surface(object *weapon, int surface_tmap, vector3 *surface_normal, float hit_dot);
...
static bool collide_weapon_and_wall(object *weapon, fix hitspeed, int hitseg, int hitwall, vector3 *hitpnt,
                                    vector3 *wall_normal, float hit_dot);
/// Prints out a marker hud message if needed.
static void collide_player_and_marker(object *playerobj, object *marker_obj, vector3 *collision_point,
                                      vector3 *collision_normal, bool f_reverse_normal, fvi_info *hit_info);
static void collide_player_and_wall(object *playerobj, float hitspeed, int hitseg, int hitwall, vector3 *hitpt,
                                    vector3 *wall_normal, float hit_dot);
static void collide_generic_and_wall(object *genericobj, float hitspeed, int hitseg, int hitwall, vector3 *hitpt,
                                     vector3 *wall_normal, float hit_dot);
static void CollideAnglesToMatrix(matrix *m, float p, float h, float b);
static vector3 *CollideExtractAnglesFromMatrix(vector3 *a, matrix *m);
static void bump_two_objects(object *object0, object *object1, vector3 *collision_point, vector3 *collision_normal,
                             int damage_flag);
static void collide_player_and_player(object *p1, object *p2, vector3 *collision_point, vector3 *collision_normal,
                                      bool f_reverse_normal, fvi_info *hit_info);
static void collide_generic_and_player(object *robotobj, object *playerobj, vector3 *collision_point,
                                       vector3 *collision_normal, bool f_reverse_normal, fvi_info *hit_info);
...
void DoWallEffects(object *weapon, int surface_tmap) {
  ...
  if (texp->flags.explosive || texp->flags.lava || texp->flags.water)
  {
    // Create some lava steam
    if (texp->flags.water || ((d3::rand() % 4) == 0)) {
  ...
  } else if (texp->flags.rubble) {
    if ((d3::rand() % 4) == 0) {
      int num_rubble = (d3::rand() % 3) + 1;
      ...
      for (int i = 0; i < num_rubble; i++) {
        ...
        visnum = VisEffectCreate(VIS_FIREBALL, RUBBLE1_INDEX + (d3::rand() % 2), weapon->roomnum, &weapon->pos);
        ...
        if (visnum >= 0) {
          ...
          if ((d3::rand() % 3) == 0) {
            ...
            vis->velocity.y() = -((d3::rand() % 200) - 30);
            vis->velocity.z() = (d3::rand() % 100) - 50;
          } else {
            vis->velocity.x() = (d3::rand() % 300) - 50;
            vis->velocity.y() = -((d3::rand() % 200) - 30);
            vis->velocity.z() = (d3::rand() % 300) - 50;
          }
...
extern void DeformTerrain(vector3 *pos, int depth, scalar size);

// Check for lava, volatile, or water surface.  If contact, make special sound & kill the weapon
void check_for_special_surface(object *weapon, int surface_tmap, vector3 *surface_normal, float hit_dot) {
  const texture_flags_t& flags = GameTextures[surface_tmap].flags;

  // Kill the weapon if the surface is volatile, lava, or water
  if (flags.explosive || flags.lava || flags.water) {
    int snd;
    ain_hear hear;
    ...
    } else if (flags.explosive || flags.lava) {
...
// Process a collision between a weapon and a wall
// Returns true if the weapon hits the wall, and false if should keep going though the wall (for breakable glass)
bool collide_weapon_and_wall(object *weapon, fix hitspeed, int hitseg, int hitwall, vector3 *hitpnt, vector3 *wall_normal,
                             float hit_dot) {  
  // mprintf(0, "Weapon hit wall, how nice.\n");

  // #ifndef RELEASE
  if (match(Weapons[weapon->id].name, "Yellow flare") && (weapon->parent_handle == Player_object->handle) &&
  ...
  const texture_flags_t& flags = GameTextures[tmap].flags;

  if (flags.forcefield && !Weapons[weapon->id].flags.matter_weapon) {
    ain_hear hear;
    hear.f_directly_player = false;
  ...
  if (!ROOMNUM_OUTSIDE(hitseg)) // First, make sure we've hit a wall, and not terrain
  {
    ...
    if (GameTextures[fp->tmap].flags.destroyable && !fp->flags.destroyed) {
      int visnum = CreateFireball(hitpnt, SHATTER_INDEX + (d3::rand() % 2), hitseg, VISUAL_FIREBALL);
    ...
    if ((fp->portal_num != -1) && GameTextures[fp->tmap].flags.breakable &&
        Weapons[weapon->id].flags.matter_weapon)
    {
  ...
  // Add a scorch
  if ((Weapons[weapon->id].scorch_handle != -1) && !flags.water && !flags.explosive && !flags.lava)
  ...
  if ((weapon->mtype.phys_info.num_bounces <= 0) && !(weapon->mtype.phys_info.flags & PF_STICK) &&
    ...
    if ((Weapons[weapon->id].flags.spawns_robot) && (Weapons[weapon->id].flags.countermeasure) &&
...
// Prints out a marker hud message if needed
void collide_player_and_marker(object *playerobj, object *marker_obj, vector3 *collision_point, vector3 *collision_normal,
                               bool f_reverse_normal, fvi_info *hit_info) {
...
void collide_player_and_wall(object *playerobj, float hitspeed, int hitseg, int hitwall, vector3 *hitpt,
                             vector3 *wall_normal, float hit_dot) {
  float volume = MAX_GAME_VOLUME;
  bool flags.explosive, flags.lava;
  ...
  const texture_flags_t& flags = GameTextures[v].flags;
  ...
  if (!flags.forcefield && !flags.explosive && !flags.lava && hitspeed > MIN_WALL_HIT_SOUND_VEL &&
      Players[playerobj->id].last_hit_wall_sound_time + MIN_PLAYER_WALL_SOUND_TIME < d3::chrono::last_update()) {
...
void collide_generic_and_wall(object *genericobj, float hitspeed, int hitseg, int hitwall, vector3 *hitpt,
                              vector3 *wall_normal, float hit_dot) {
  bool flags.explosive, flags.lava;
  bool flags.forcefield;
  float volume = MAX_GAME_VOLUME;
  ...
  texture_flags_t flags = GameTextures[v].flags;

  if (IS_GUIDEBOT(genericobj)) {
    flags.forcefield = false;
...
vector3 *CollideExtractAnglesFromMatrix(vector3 *a, matrix *m) {
...
void ConvertEulerToAxisAmount(vector3 *e, vector3 *n, float *w) {
  ...
  // This is from Graphics Gems 1 p.467  I am converting from a angle vector3
...
void bump_obj_against_fixed(object *obj, vector3 *collision_point, vector3 *collision_normal) {
  Q_ASSERT(std::isfinite(obj->mtype.phys_info.rotvel.x()));
  Q_ASSERT(std::isfinite(obj->mtype.phys_info.rotvel.y()));
  Q_ASSERT(std::isfinite(obj->mtype.phys_info.rotvel.z()));
  Q_ASSERT(std::isfinite(obj->mtype.phys_info.velocity.x()));
  Q_ASSERT(std::isfinite(obj->mtype.phys_info.velocity.y()));
  Q_ASSERT(std::isfinite(obj->mtype.phys_info.velocity.z()));

  if (!IsOKToApplyForce(obj))
    return;

  vector3 r1 = *collision_point - obj->pos;
  vector3 w1;
  vector3 n1;
  float temp1;
  ...
  vector3 cmp1 = obj->mtype.phys_info.rotvel * o_t1;
  ...
  vector3 p1 = obj->mtype.phys_info.velocity + w1;
  float v_rel;
  ...
  vector3 jcn = j * (*collision_normal);
...
void bump_two_objects(object *object0, object *object1, vector3 *collision_point, vector3 *collision_normal,
                      int damage_flag) {
  object *t = nullptr;
  ...
  Q_ASSERT(std::isfinite(object1->mtype.phys_info.rotvel.x()));
  Q_ASSERT(std::isfinite(object1->mtype.phys_info.rotvel.y()));
  Q_ASSERT(std::isfinite(object1->mtype.phys_info.rotvel.z()));
  Q_ASSERT(std::isfinite(object0->mtype.phys_info.rotvel.x()));
  Q_ASSERT(std::isfinite(object0->mtype.phys_info.rotvel.y()));
  Q_ASSERT(std::isfinite(object0->mtype.phys_info.rotvel.z()));
  Q_ASSERT(std::isfinite(object1->mtype.phys_info.velocity.x()));
  Q_ASSERT(std::isfinite(object1->mtype.phys_info.velocity.y()));
  Q_ASSERT(std::isfinite(object1->mtype.phys_info.velocity.z()));
  Q_ASSERT(std::isfinite(object0->mtype.phys_info.velocity.x()));
  Q_ASSERT(std::isfinite(object0->mtype.phys_info.velocity.y()));
  Q_ASSERT(std::isfinite(object0->mtype.phys_info.velocity.z()));

  vector3 r1 = *collision_point - object0->pos;
  vector3 r2 = *collision_point - object1->pos;
  vector3 w1;
  vector3 w2;
  vector3 n1;
  vector3 n2;
  ...
  vector3 cmp1 = object0->mtype.phys_info.rotvel * o_t1;
  vector3 cmp2 = object1->mtype.phys_info.rotvel * o_t2;
  ...
  vector3 p1 = object0->mtype.phys_info.velocity + w1;
  vector3 p2 = object1->mtype.phys_info.velocity + w2;
  float v_rel;
...
void collide_player_and_player(object *p1, object *p2, vector3 *collision_point, vector3 *collision_normal,
                               bool f_reverse_normal, fvi_info *hit_info) {
  if (f_reverse_normal)
    *collision_normal *= -1.0f;

  vector3 rvel = p1->mtype.phys_info.velocity - p2->mtype.phys_info.velocity;
...
void collide_generic_and_player(object *robotobj, object *playerobj, vector3 *collision_point, vector3 *collision_normal,
                                bool f_reverse_normal, fvi_info *hit_info) {
  if (f_reverse_normal)
    *collision_normal *= -1.0f;
...
void collide_generic_and_weapon(object *robotobj, object *weapon, vector3 *collision_point, vector3 *collision_normal,
  ...
  if (ApplyDamageToGeneric(robotobj, weapon, damage_type, damage_to_apply)) {
    ...
    if (!electrical) {
      ...
      CreateRandomLineSparks(3 + d3::rand() % 6, &weapon->pos, weapon->roomnum, color);
    }
  }
#if 0	
	if(Demo_flags == DF_RECORDING)
...
void collide_player_and_weapon(object *playerobj, object *weapon, vector3 *collision_point, vector3 *collision_normal,
                               bool f_reverse_normal, fvi_info *hit_info) {
  object *parent_obj;
  float damage_to_apply;
  uint8_t electrical = Weapons[weapon->id].flags.electrical ? 1 : 0;
...
bool collide_object_with_wall(object *A, float hitspeed, int hitseg, int hitwall, vector3 *hitpt, vector3 *wall_normal,
  ...
  default:
    ...
    //		LOG_ERROR( "Unhandled object type hit wall in collide.c\n" );
  }
