...
static vector3 Original_pos;
static matrix Original_orient;

static vector3 fvi_move_fvec;
static vector3 fvi_move_uvec;
...
static inline bool ns_movement_manual_AABB(vector3 *min_xyz, vector3 *max_xyz);
...
static void CollidePolygonModel(vector3 *pos, matrix *orient, int model_num, float *normalized_time,
...
static vector3 ns_min_xyz;
static vector3 ns_max_xyz;

inline void ns_compute_movement_AABB() {
  vector3 delta_movement = *fvi_query_ptr->p1 - *fvi_query_ptr->p0;
  vector3 offset_vec;
...
inline bool ns_movement_manual_AABB(vector3 *min_xyz, vector3 *max_xyz) {
  bool overlap = true;
  ...
  if (ns_movement_manual_AABB(&sm->min, &sm->max)) {
    ...
    for (i = 0; i < sm->num_faces; i++) {
      if (ns_movement_manual_AABB(&sm->face_min[i], &sm->face_max[i])) {
        ...
        if (fp->texnum >= 0 &&
            !GameTextures[pm->textures[fp->texnum]].flags.fly_thru &&
            !GameTextures[pm->textures[fp->texnum]].flags.pass_thru)
        {
...
void newstyle_StartInstanceMatrix(vector3 *pos, matrix *orient) {
  ...
  Q_ASSERT(instance_depth < MAX_INSTANCE_DEPTH);
...
void CollidePolygonModel(vector3 *pos, matrix *orient, int model_num, float *normalized_time, uint32_t f_render_sub) {
  poly_model *po;

  Q_ASSERT(Poly_models[model_num].used);
  Q_ASSERT(Poly_models[model_num].new_style);
...
bool PolyCollideObject(object *obj) {
...
#endif
  vector3 temp_pos = Original_pos = View_position;
  ...
  if (fvi_check_param) {
    ...
    vector3 x = *fvi_query_ptr->p1 - *fvi_query_ptr->p0;
    vm_NormalizeVector(&x);
