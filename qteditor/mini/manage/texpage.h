...
int mng_SetAndLoadTexture(mngs_texture_page *texpage, struct CFILE* infile);

// Reads in a texture page from the local table file, superseding any texture
// already in RAM with that same name
void mng_LoadLocalTexturePage(struct CFILE* );

// Reads in a page off the net
void mng_LoadNetTexturePage(struct CFILE* , bool overlay = false);

// Reads in the texpage named "name" into texpage struct
// Returns 0 on error, else 1 if all is good
int mng_FindSpecificTexPage(const std::string &name, mngs_texture_page *texpage, int offset = 0);

// Given a texpage and a texture handle, attempts to make texture n correspond to
// to the texpage.
int mng_AssignTexPageToTexture(mngs_texture_page *texpage, int n, struct CFILE* infile = NULL);
...
int mng_GetGuaranteedTexturePage(const std::string &name, struct CFILE* infile = NULL);
