...
static vector3 *TS_View_position;

static vector3 TS_PreRows[TERRAIN_DEPTH];
static vector3 TS_FVectorAdd, TS_RVectorAdd;

static uint8_t Terrain_y_flags[256];
static vector3 Terrain_y_cache[256];
...
void PreRotateTerrain() {
  vector3 rvec = {1 * TERRAIN_SIZE, 0, 0};
  vector3 fvec = {0, 0, 1 * TERRAIN_SIZE};

  TS_FVectorAdd = fvec * *TS_View_matrix;
  TS_RVectorAdd = rvec * *TS_View_matrix;

  vector3 start_vec = {0.0f, 0.0f, 0.0f};
  vector3 temp_vec = start_vec - *TS_View_position;
...
static inline vector3 *GetDYVector(int h) {
  vector3 *dyp;

  dyp = &Terrain_y_cache[h];

  if (!Terrain_y_flags[h]) {
    vector3 up_vector = {0, h * TERRAIN_HEIGHT_INCREMENT, 0};
...
static inline void GetPreRotatedPointFast(vector3 *dest, int x, int z, int yvalue) {
  Q_ASSERT(yvalue >= 0 && yvalue <= 255);
...
void GetSpecialRotatedPoint(g3Point *dest, int x, int z, float yvalue) {
  ...
  vector3 up_vector = {0, yvalue, 0};
  vector3 dyp = up_vector * *TS_View_matrix;
...
void Terrain_start_frame(vector3 *eye, matrix *view_orient) {
  ...
  TS_Lambda = (float)height / (2 * ViewDistance1 * D3_DEFAULT_ZOOM);//Render_zoom);
...
int GetVisibleTerrain(vector3 *eye, matrix *view_orient) {
  int cellcount = 0;
  int i, t, count;
...
__inline void CheckCellOccupancy(int x, int y, int *ccount, uint8_t lod) {
  ...
  if (*ccount >= MAX_CELLS_TO_RENDER) {
    LOG_WARNING("Trying to render too many cells!  Cell limit=%d", MAX_CELLS_TO_RENDER);
#ifndef NEWEDITOR
