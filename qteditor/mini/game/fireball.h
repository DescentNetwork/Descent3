...
struct fireball {
  std::string name; // The filename of this animation
...
int CreateFireball(vector3 *pos, int fireball_num, int roomnum, int realtype = VISUAL_FIREBALL);
...
extern int CreateGravityField(vector3 *pos, int roomnum, float size, float time, int parent_handle);
...
int CreateBlastRing(vector3 *pos, int index, float lifetime, float max_size, int roomnum, int force_blue = 0);

// Creates a standard blast ring for an object
int CreateObjectBlastRing(object *objp);

// Creates a smolding smoke to be drawn
int CreateSmolderingObject(vector3 *pos, int index, float lifetime, float max_size, int roomnum);

// Draws a blast ring
void DrawBlastRingObject(object *obj);

// Draws a colored alpha disk...useful for cool lighting effects
void DrawColoredDisk(vector3 *pos, float r, float g, float b, float inner_alpha, float outer_alpha, float size,
                     uint8_t saturate = 0, uint8_t lod = 1);

// Draws a colored alpha ring...useful for cool lighting effects
void DrawColoredRing(vector3 *pos, float r, float g, float b, float inner_alpha, float outer_alpha, float size,
...
int CreateCustomFireballObject(vector3 *pos, int fireball_num, int tex_handle, int roomnum);
...
void DrawSphere(vector3 *pos, float r, float g, float b, float alpha, float size, int texture, uint8_t saturate = 1);

// Creates end points that simulate lightning
void CreateLightningRodPositions(vector3 *src, vector3 *dest, vector3 *world_vecs, int num_segments, float rand_mag,
                                 bool do_flat);

// Draws a glowing cone of light using a bitmap
void DrawColoredGlow(vector3 *pos, float r, float g, float b, float size);
