...
void DeformTerrain(vector3 *pos, int depth, float size) {
...
void GenerateTerrainLight() {
  ...
  vector3 camera_light = Terrain_sky.lightsource;
...
void SetupSky(float radius, int flags, uint8_t randit) {
  ...
  for (i = 0; i < 6; i++) {
    ...
    for (t = 0; t < MAX_HORIZON_PIECES; t++) {
      vector3 *vec = &Terrain_sky.horizon_vectors[t][i];
