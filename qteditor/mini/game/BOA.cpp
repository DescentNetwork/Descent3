...
bool BOA_PassablePortal(int room, int portal_index, bool f_for_sound, bool f_making_robot_path_invalid_list) {
  ...
  if (room >= Rooms.size() && room < Rooms.size() + BOA_num_terrain_regions) {
    int tr;
  ...
  if (!BOA_f_making_boa) {
    if (BOA_cost_array[room][portal_index] < 0.0f && !(room < Rooms.size() && Rooms[room].flags.external))
      return false;
...
std::optional<uint32_t> BOA_DetermineStartRoomPortal(int start_room, vector3 *start_pos, int end_room, vector3 *end_pos,
  ...
  if (start_room == -1 || end_room == -1)
    return std::nullopt;
  if (start_room >= Rooms.size() && end_room >= Rooms.size())
    return std::nullopt;

  start_room = BOA_INDEX(start_room);
  end_room = BOA_INDEX(end_room);

  if (start_room < Rooms.size()) {
...
bool BOA_ComputeMinDist(int start_room, int end_room, float max_check_dist, float *dist, int *num_blockages) {
  *dist = 0.0f;
  ...
  if (end_room == Rooms.size() && start_room >= Rooms.size()) {
    return true;
  }
  ...
  if (start_room >= Rooms.size() + BOA_num_terrain_regions ||
      end_room >= Rooms.size() + BOA_num_terrain_regions) {
    return false;
  }

  if (start_room < Rooms.size() && !Rooms[start_room].used)
    return false;

  if (end_room < Rooms.size() && !Rooms[end_room].used)
    return false;
...
bool BOA_IsSoundAudible(int start_room, int end_room) {
  ...
  if ((!ROOMNUM_OUTSIDE(s_index)) && s_index < Rooms.size()) {
    if (!Rooms[s_index].used) {
      return false;
    }
  } else if (ROOMNUM_OUTSIDE(s_index)) {
    s_index = Terrain_seg[start_room].flags.region + Rooms.size();
  } else {
    Q_ASSERT(s_index < Rooms.size() + MAX_BOA_TERRAIN_REGIONS);
  }

  if ((!ROOMNUM_OUTSIDE(e_index)) && e_index < Rooms.size()) {
    if (!Rooms[e_index].used) {
      return false;
    }
  } else if (ROOMNUM_OUTSIDE(e_index)) {
    e_index = Terrain_seg[end_room].flags.region + Rooms.size();
  } else {
...
void compute_terrain_region_info() {
  ...
  while (!done) {
    if (t_region >= MAX_BOA_TERRAIN_REGIONS) {
      QMessageBox::warning(nullptr, QString("%1 warning").arg(__func__), "This terrain has too many regions!\nAI will not work correctly outside!\nSaturate useless areas!");
      break;
    }
  ...
  if (f_warning) {
    QMessageBox::warning(nullptr, QString("%1 warning").arg(__func__),
...
void compute_costs() {
  ...
  for (i = Rooms.size(); i < Rooms.size() + BOA_num_terrain_regions; i++) {
    int j;
    for (j = 0; j < BOA_num_connect[i]; j++) {
...
void FindPath(int i, int j) {
  ...
  Q_ASSERT(start_node->roomnum < Rooms.size() + BOA_num_terrain_regions);

  while ((cur_node = PQPath.pop())) {
    node_list[BOA_INDEX(cur_node->roomnum)] = cur_node;
    Q_ASSERT(BOA_INDEX(cur_node->roomnum) >= 0 &&
           BOA_INDEX(cur_node->roomnum) < Rooms.size() + MAX_BOA_TERRAIN_REGIONS);
    ...
    if (i > j) {
      ...
      if (cur_node->roomnum < Rooms.size()) {
      ...
      for (counter = 0; counter < num_portals; counter++) {
        ...
        if ((next_room < Rooms.size()) && Rooms[next_room].flags.external) {
          Q_ASSERT(cur_node->roomnum < Rooms.size());
          ...
          Q_ASSERT(next_room < Rooms.size() + BOA_num_terrain_regions);
        }
        ...
        if (list_item == NULL) {
          ...
          Q_ASSERT(list_item->roomnum < Rooms.size() + BOA_num_terrain_regions);
        } else {
...
void compute_next_segs() {
  ...
  for (i = 0; i < Rooms.size() + MAX_BOA_TERRAIN_REGIONS; i++) {
    ...
    if (i >= Rooms.size() + BOA_num_terrain_regions)
      continue;

    for (j = ((int)Rooms.size() - 1) + MAX_BOA_TERRAIN_REGIONS; j >= 0; j--) {
      if (j < Rooms.size() && (!Rooms[j].used))
        continue;

      if (j < Rooms.size() && Rooms[j].flags.external)
        continue;

      if (j >= Rooms.size() + BOA_num_terrain_regions)
        continue;
...
void compute_blockage_info() {
  ...
  for (i = 0; i < Rooms.size() + BOA_num_terrain_regions; i++) {
    if (i < Rooms.size() && (!Rooms[i].used))
      continue;
    ...
    for (j = 0; j < Rooms.size() + BOA_num_terrain_regions; j++) {
      int cur_room = i;
...
bool IsPathPointValid(int room, vector3 *pos) {
  // vector3 c_pnt = Rooms[room].path_pnt;
  int i;

  if (Rooms[room].flags.external)
    return true;
  ...
  for (i = 0; i < Rooms[room].num_portals; i++) {
    ...
    vector3 portal_pnt = Rooms[room].portals[i].path_pnt;
...
void ValidateRoomPathPoint(int room, char *message, int len) {
  ...
  vector3 diff = (Rooms[room].max_xyz - Rooms[room].min_xyz) / MAX_SUBDIVISIONS;

  for (i = 1; i < MAX_SUBDIVISIONS - 1; i++) {
    for (j = 1; j < MAX_SUBDIVISIONS - 1; j++) {
      for (k = 1; k < MAX_SUBDIVISIONS - 1; k++) {
        vector3 t_pnt = Rooms[room].min_xyz;
        t_pnt.x() += diff.x() * i;
        t_pnt.y() += diff.y() * j;
        t_pnt.z() += diff.z() * k;
...
void MakeBOAVisTable(bool from_lighting) {
  ...
  if (cur_check == BOA_vis_checksum) {
    ...
    if (!from_lighting)
      QMessageBox::information(nullptr, "Success", "BOA vis table already computed.");
    return;
  }
  ...
  for (i = 0; i < Rooms.size(); i++) {
    ...
    while (stack_count > 0) {
      ...
      for (t = 0; t < rp->num_portals && !done; t++) {
        ...
        vector3 src_upper_left, src_center;
        matrix src_matrix;
        vector3 src_verts[MAX_VERTS_PER_FACE], *src_vertp[MAX_VERTS_PER_FACE];
        ...
        for (j = 0; j < Rooms[check_room].num_portals && !done; j++) {
          ...
          vector3 dest_upper_left, dest_center;
          matrix dest_matrix;
          vector3 dest_verts[MAX_VERTS_PER_FACE], *dest_vertp[MAX_VERTS_PER_FACE];
  ...
  if (!from_lighting)
    QMessageBox::information(nullptr, "Success", "BOA vis table computed.");
  ...
  if (cur_check == BOA_AABB_checksum && !f_full) {
    ...
    for (i = 0; i < Rooms.size(); i++) {
      if (Rooms[i].used) {
        ...
        int best = 0;
        vector3 diff = s_max_xyz[0] - s_min_xyz[0];
      ...
      if (Rooms[i].used) {
        ...
        for (count = 0; count < rp->num_faces; count++) {
          ...
          if (f_part == false) {
            ...
            Q_ASSERT(slot > 0 && slot < rp->num_bbf_regions);
          }
        }
      }
    }
    ...
    for (i = 0; i < Rooms.size(); i++) {
      if (Rooms[i].used) {
        ...
        for (j = 0; j < original_bbf_regions; j++) {
          if (rp->num_bbf[j] > 15) {
            // Test 3 split type
            vector3 split = (rp->bbf_list_min_xyz[j] + rp->bbf_list_max_xyz[j]) / 2.0f;
            int num_faces[3][3];
