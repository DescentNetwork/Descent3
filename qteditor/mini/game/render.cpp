...
vector3 Room_fog_plane, Room_fog_portal_vert;
...
vector3 Global_alter_vec = {19, -19, 19};
...
void MakePointsFromMinMax(vector3 *corners, vector3 *minp, vector3 *maxp) {
  corners[0].x() = minp->x();
  corners[0].y() = maxp->y();
  corners[0].z() = minp->z();
  corners[1].x() = maxp->x();
...
void ReflectRay(vector3 *dest, vector3 *src, vector3 *mirror_norm) {
  *dest = *src;
  scalar d = vm_Dot3Product(*dest, *mirror_norm);
  vector3 upvec = d * *mirror_norm;
  *dest -= (2.0f * upvec);
}
...
void MarkFacesForRendering(int roomnum, clip_wnd *wnd) {
  ...
  if (rp->flags.door) {
    ...
    if (rp->mirror_face == -1 || !Detail_settings.Mirrored_surfaces) {
      ...
      if (rp->flags.mirror_visible) // Mirror is visible, just mark all faces as visible
      {
        fp = &rp->faces[0];
        for (i = 0; i < rp->num_faces; i++, fp++) {
          fp->flags.visible = true;
...
void RotateAllExternalRooms() {
  ...
  if (N_external_rooms == -1) {
    ...
    Q_ASSERT(N_external_rooms < MAX_EXTERNAL_ROOMS); // Get Jason if hit this
...
void CheckFogPortalExtents(int roomnum, int portalnum) {
  ...
  vector3 *vec = &rp->verts[fp->face_verts[0]];
...
void BuildRoomListSub(int start_room_num, clip_wnd *wnd, int depth) {
  ...
  Q_ASSERT(N_render_rooms < MAX_RENDER_ROOMS);
#ifndef RELEASE
  Mine_depth++;
#endif
  ...
  for (t = 0; t < rp->num_portals; t++) {
    ...
    if (rp->flags.external && Rooms[croom].flags.door)
...
void RenderSpecularFacesFlat(room *rp) {
  ...
  for (i = 0; i < Num_specular_faces_to_render; i++) {
    ...
    w = static_cast<int>(lm_w(lm_handle).value_or(255));
    h = static_cast<int>(lm_h(lm_handle).value_or(255));
...
texel = data[int_v][int_u];
      ...
      vector3 subvec = Viewer_eye - rp->verts[fp->face_verts[vn]];
      ...
      if (!(rp->flags.external)) {
        ...
        vector3 incident_norm = rp->verts[fp->face_verts[vn]] - Terrain_sky.satellite_vectors[0];
        vm_NormalizeVectorFast(&incident_norm);

        float d = vm_Dot3Product(incident_norm, fp->normal);
        vector3 upvec = d * fp->normal;
        incident_norm -= (2 * upvec);
        float dotp = vm_Dot3Product(subvec, incident_norm);
        if (dotp > 1)
          dotp = 1;
  ...
  for (int i = 0; i < Num_fog_faces_to_render; i++) {
    ...
    for (vn = 0; vn < fp->num_verts; vn++) {
      ...
      if (Room_fog_plane_check == 0) {
        // Outside of the room
        vector3 *vec = &rp->verts[fp->face_verts[vn]];
        ...
        vector3 subvec = *vec - Viewer_eye;
        scalar t = Room_fog_eye_distance / (Room_fog_eye_distance - dist);
        vector3 portal_point = Viewer_eye + (t * subvec);
...
void RenderFace(room *rp, int facenum) {
  ...
  if (rp->flags.triangulate)
    do_triangle_test = 1;
  if (!Render_mirror_for_room && Detail_settings.Specular_lighting && (GameTextures[fp->tmap].flags.smooth_specular) &&
  ...
  if (GameTextures[fp->tmap].slide_u != 0) {
    int int_time = d3::chrono::last_update() / GameTextures[fp->tmap].slide_u;
    float norm_time = d3::chrono::last_update() - (int_time * GameTextures[fp->tmap].slide_u);
    norm_time /= GameTextures[fp->tmap].slide_u;
    uchange = norm_time;
  }
  if (GameTextures[fp->tmap].slide_v != 0) {
    int int_time = d3::chrono::last_update() / GameTextures[fp->tmap].slide_v;
    float norm_time = d3::chrono::last_update() - (int_time * GameTextures[fp->tmap].slide_v);
  ...
  if (NoLightmaps) {
    ...
    if (fp->flags.lightmap) {
      int lm_handle = LightmapInfo[fp->lmi_handle].lm_handle;
      const std::vector<std::vector<uint16_t>> &data = lm_data(lm_handle);
      int w = static_cast<int>(lm_w(lm_handle).value_or(255));
      int h = static_cast<int>(lm_h(lm_handle).value_or(255));
...
// If searching,
  ...
  if (face_cc.cc_or) // Possible triangulate this face because it is off screen somewhat
  {
    if (Room_light_val < 1.0)
      do_triangle_test = 1;
    if (Detail_settings.Fog_enabled && (rp->flags.fog))
      do_triangle_test = 1;
    if (spec_face)
      do_triangle_test = 1;
  }
...
// if (Render_mirror_for_room && (GameTextures[fp->tmap].flags.alpha))
// rend_SetZBufferWriteMask (1);
#ifdef EDITOR
  ...
  if (Outline_lightmaps) {
    ...
    if (fp == &Curroomp->faces[Curface] && (fp->flags.lightmap)) {
      ...
      int w = static_cast<int>(lm_w(lmi->lm_handle).value_or(255));
      int h = static_cast<int>(lm_h(lmi->lm_handle).value_or(255));
      for (int i = 0; i < w * h; i++) {
        int t;
        g3Point epoints[20];
        vector3 evec[20];
...
void ComputeRoomPulseLight(room *rp) {
  if (rp->pulse_time == 0 || In_editor_mode)
    ...
    int int_time = (int)((d3::chrono::last_update() + add_time) / (ptime * 2));
    float left_time = (d3::chrono::last_update() + add_time) - (int_time * ptime * 2);
...
void RenderSingleLightGlow(int index) {
  ...
  vector3 center = LightGlows[index].center;

  // Get alpha of light
  vector3 tvec = Viewer_eye - rp->verts[fp->face_verts[0]];
...
void RenderSingleLightGlow2(int index) {
  ...
  vector3 center = LightGlows[index].center;
  vector3 corona_pos = center + (fp->normal * size);
...
void BuildMirroredRoomListSub(int start_room_num, clip_wnd *wnd) {
  ...
  for (t = 0; t < rp->num_portals; t++) {
    ...
    if (!external_door_hack) {
      ...
      vector3 tvec = Viewer_eye - temp_vec;
      if (vm_Dot3Product(tvec, incident_norm) <= 0)
        continue; // not facing
    }
    ...
    for (i = 0; i < nv; i++) {
      vector3 temp_vec;
      vector3 *vec = &rp->verts[fp->face_verts[i]];
      float dist_from_mirror =
          vec->x() * mirror_norm->x() + vec->y() * mirror_norm->y() + vec->z() * mirror_norm->z() + mirror_dist;
      // dest_vecs contains the point on the other side of the mirror (ie the reflected point)
      temp_vec = *vec - (*mirror_norm * (dist_from_mirror * 2));
...
void RenderRoomObjects(room *rp) {
  ...
  Q_ASSERT(objnum == -1); // if not -1, ran out of space in render_order[]
  Q_ASSERT(visnum == -1); // if not -1, ran out of space in render_order[]
  // Sort the objects
  ...
  if (UseHardware && Render_mirror_for_room) {
    ...
    for (i = n_objs - 1; i >= 0; i--) {
      ...
      if (obj_sort_list[i].vis_effect) {
        ...
        vector3 save_vec = vis->pos;
        vector3 save_end_vec = vis->end_pos;
        vector3 *vec = &vis->pos;
        vector3 *end_vec = &vis->end_pos;
        float dist_from_mirror = vec->x() * norm->x() + vec->y() * norm->y() + vec->z() * norm->z() + mirror_dist;
        // dest_vecs contains the point on the other side of the mirror (ie the reflected point)
        ...
        vector3 save_vec = objp->pos;
        matrix save_orient = objp->orient;
        matrix temp_mat;

        vector3 *vec = &objp->pos;
        float dist_from_mirror = vec->x() * norm->x() + vec->y() * norm->y() + vec->z() * norm->z() + mirror_dist;
        // dest_vecs contains the point on the other side of the mirror (ie the reflected point)
  ...
  if (UseHardware) {
    ...
    for (index = Rooms[roomnum].vis_effects; index != -1; index = VisEffects[index].next) {
      ...
      if (IsPointVisible(&VisEffects[index].pos, VisEffects[index].size, &zdist)) {
        // Stuff viseffects into our postrender list (disabled: post-render system not ported)
        // if (Num_postrenders < MAX_POSTRENDERS) {
...
void RenderMirrorRooms() {
  ...
  for (i = 0; i < static_cast<int>(Mirror_rooms.size()); i++) {
    ...
    if (do_mirror_face) {
      if (!(GameTextures[rp->faces[rp->mirror_face].tmap].flags.alpha))
        do_mirror_face = false;
    }
    if (do_mirror_face) {
      int on_screen = 0;
  ...
  for (i = 0; i < static_cast<int>(Mirror_rooms.size()); i++) {
    ...
    for (int k = 0; k < rp->num_faces; k++) {
      ...
      if (this_fp->tmap == fp->tmap) {
        ...
        // DrawPostrenderFace(Mirror_rooms[i], rp->mirror_face, false); // post-render system not ported
...
void RenderMine(int viewer_roomnum, int flag_automap, int called_from_terrain) {
  ...
  if (Must_render_terrain && !Called_from_terrain && !(In_editor_mode && Render_inside_only)) {
    ...
    if ((Terrain_sky.flags.fog) && (UseHardware || (!UseHardware && Lighting_on))) {
      rend_SetFogState(1);
...
#define STATE_PUSH(val)                                                                                                \
  {                                                                                                                    \
    ...
    state_stack_counter++;                                                                                             \
    Q_ASSERT(state_stack_counter < 2000);                                                                                \
  }
#define STATE_POP()                                                                                                    \
  {                                                                                                                    \
    state_stack_counter--;                                                                                             \
