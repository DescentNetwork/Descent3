...
static void ApplyVolumeLightToObject(vector3 *pos, object *obj, float light_dist, float red_scale, float green_scale,
                                     float blue_scale, vector3 *light_direction, float dot_range);
static void ApplyLightingToObjects(vector3 *pos, int roomnum, float light_dist, float red_scale, float green_scale,
                                   float blue_scale, vector3 *light_direction, float dot_range);
...
uint8_t Float_to_ubyte(float fnum) {
  ...
  if (fnum < 0 || fnum > 1.0) {
    LOG_WARNING("ERROR: Trying to get value in Float_to_ubyte that is %f!", fnum);
    return 0;
  }
...
int GetFreeDynamicLightmap(int w, int h) {
  ...
  Q_ASSERT(Dynamic_lightmaps[n].mem_ptr);
...
void ApplyLightingToExternalRoom(vector3 *pos, int roomnum, float light_dist, float red_scale, float green_scale,
  ...
  for (i = 0; i < rp->num_faces; i++) {
    ...
    if (light_direction) {
      ...
      vector3 norm_vec = *pos - rp->verts[fp->face_verts[0]];
    ...
    vector3 touch_vector = *pos - (lmi_ptr->normal * dist_from_plane);
    ...
    for (int y = 0; y < height; y++, base_vector -= (facematrix.uvec * lmi_ptr->yspacing), texel_num += lmw) {
      ...
      for (int x = 0; x < width; x++, element_vec += (facematrix.rvec * lmi_ptr->xspacing)) {
        ...
        if (light_direction) {
          vector3 lsubvec = element_vec - *pos;
...
void StartLightingInstance(vector3 *pos, matrix *orient) {
  vector3 tempv;

  Q_ASSERT(Light_instance_depth < MAX_SUBOBJECTS);
...
void DoneLightingInstance() {
  ...
  Q_ASSERT(Light_instance_depth >= 0);
...
void ApplyLightingToSubmodel(object *obj, poly_model *pm, bsp_info *sm, float light_dist, float red_scale,
  ...
  vector3 light_pos = Light_position;
  ...
  for (i = 0; i < sm->num_faces; i++) {
    ...
    // Get upper left vector3 of face
    vector3 base_vector = lmi_ptr->upper_left;
    vector3 subvec = light_pos - lmi_ptr->upper_left;
    float dist_from_plane = fabs(vm_DotProduct(&subvec, &lmi_ptr->normal));
    ...
    if (Use_light_direction) {
      // If this is a directional light, check for backfaces
      vector3 norm_vec = light_pos - base_vector;
    ...
    // Get the area of effect for this light
    float area_of_effect = (1.0 - (dist_from_plane / light_dist)) * light_dist;
    vector3 touch_vector = light_pos - (lmi_ptr->normal * dist_from_plane);
    ...
    for (int y = 0; y < height; y++, base_vector -= (facematrix.uvec * lmi_ptr->yspacing), texel_num += lmw) {
      ...
      for (int x = 0; x < width; x++, element_vec += (facematrix.rvec * lmi_ptr->xspacing)) {
        ...
        if (Use_light_direction) {
          vector3 lsubvec = element_vec - light_pos;
          vm_NormalizeVectorFast(&lsubvec);
...
void ApplySpecularLightingToWall (room *rp,face *fp,vector3 *pos,float light_dist,float dist_from_plane,float
...
void ApplyVolumeLightToObject(vector3 *pos, object *obj, float light_dist, float red_scale, float green_scale,
                              float blue_scale, vector3 *light_direction, float dot_range) {
  vector3 subvec = *pos - obj->pos;
...
void ApplyLightingToObjects(vector3 *pos, int roomnum, float light_dist, float red_scale, float green_scale,
                            float blue_scale, vector3 *light_direction, float dot_range) {
...
// directional light
void ApplyLightingToRooms(vector3 *pos, int roomnum, float light_dist, float red_scale, float green_scale,
                          float blue_scale, vector3 *light_direction, float dot_range) {
...
void ApplyLightingToTerrain(vector3 *pos, int cellnum, float light_dist, float red_scale, float green_scale,
                            float blue_scale, vector3 *light_direction, float dot_range) {
  ...
  LOG_WARNING_IF(num_cells == MAX_DYNAMIC_CELLS,
                 "One object has hit %d terrain cells in lighting!",
                 MAX_DYNAMIC_CELLS);
  ...
  for (i = 0; i < num_cells; i++) {
    ...
    vector3 subvec = *pos - tpos;

    float dist = vm_GetMagnitudeFast(&subvec);
    float scalar = 1.0 - (dist / light_dist);

    if (light_direction) {
      vector3 lsubvec = -subvec;
      lsubvec /= dist;
      float dp = vm_DotProduct(&lsubvec, light_direction);
      if (dp < dot_range) {
        continue;
        ...
        for (int y=0;y<yres;y++,base_vector-=(facematrix.uvec*yspacing),texel_num+=xres)
        {
                ...
                for (int x=0;x<xres;x++,element_vec+=(facematrix.rvec*xspacing))
                {
                        ...
                        for (int i=0;i<num;i++)
                        {
                                ...
                                vector3 subvec=*pos-element_vec;
        ...
        for (int y=0;y<height;y++,base_vector.y-=lmi_ptr->yspacing,texel_num+=xres)
        {
                ...
                for (int x=0;x<width;x++,element_vec.x+=lmi_ptr->xspacing)
                {
                        ...
                        for (int i=0;i<num;i++)
                        {
                                ...
                                vector3 subvec=eye_pos-element_vec;
                                vm_NormalizeVectorFast (&subvec);

                                vector3 incident_norm=element_vec-light_pos[i];
                                vm_NormalizeVectorFast (&incident_norm);

                                float d=incident_norm * norm;
                                vector3 upvec=d * norm;
                                incident_norm-=(2*upvec);
...
void DestroyLight(int roomnum, int facenum) {
  ...
  LOG_DEBUG("r=%f g=%f b=%f", red_scale, green_scale, blue_scale);
  ...
  for (i = 0; i < num_faces; i++) {
    ...
    vector3 touch_vector = center - (lmi_ptr->normal * dist_from_plane);
