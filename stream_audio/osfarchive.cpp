...
bool OSFArchive::Open(const std::filesystem::path &filename, bool write) {
  ...
  try {
    ...
    if (strcmp(OSF_TAG, tag) != 0) {
      LOG_WARNING("Illegal OSF file format for %s.", filename.u8string().c_str());
      cfclose(m_fp);
      m_fp = NULL;
      return false;
    }
    ...
    if (m_type == OSF_DIGITAL_STRM) {
      ...
      LOG_WARNING("Unsupported OSF file type in %s!", filename.u8string().c_str());
      cfclose(m_fp);
      m_fp = NULL;
      return false;
    }
    ...
    if (!cf_ReadBytes((uint8_t *)m_name, OSF_HDR_TITLE_LEN, m_fp)) {
      LOG_WARNING("Stream title not found for %s.", filename.u8string().c_str());
      cfclose(m_fp);
      m_fp = NULL;
      return false;
    }
