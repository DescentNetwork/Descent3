...
int mng_AssignWeaponPageToWeapon(mngs_weapon_page *weaponpage, int n, CFILE *infile) {
  ...
  if (img_handle < 0) {
    LOG_ERROR("Couldn't load bitmap '%s' in AssignWeaponPage...", weaponpage->hud_image_name);
  ...
  if (img_handle < 0) {
    LOG_ERROR("Couldn't load bitmap/model '%s' in AssignWeaponPage...", weaponpage->fire_image_name);
  ...
  if (stricmp(weaponpage->explode_image_name, "INVALID NAME") && weaponpage->explode_image_name[0] != 0) {
    ...
    if (img_handle < 0) {
      LOG_ERROR("Couldn't load bitmap/model '%s' in AssignWeaponPage...", weaponpage->explode_image_name);
  ...
  if (stricmp(weaponpage->particle_name, "INVALID NAME") && weaponpage->particle_name[0] != 0) {
    ...
    if (img_handle < 0) {
      LOG_ERROR("Couldn't load bitmap/model '%s' in AssignWeaponPage...", weaponpage->particle_name);
  ...
  if (stricmp(weaponpage->spawn_name, "INVALID NAME") && weaponpage->spawn_name[0] != 0) {
    ...
    if (img_handle < 0) {
      LOG_ERROR("Couldn't load spawn weapon '%s' in AssignWeaponPage...", weaponpage->spawn_name);
  ...
  if (stricmp(weaponpage->alternate_spawn_name, "INVALID NAME") && weaponpage->alternate_spawn_name[0] != 0) {
    ...
    if (img_handle < 0) {
      LOG_ERROR("Couldn't load alternate spawn weapon '%s' in AssignWeaponPage...",
  ...
  if (stricmp(weaponpage->robot_spawn_name, "INVALID NAME") && weaponpage->robot_spawn_name[0] != 0) {
    ...
    if (img_handle < 0) {
      LOG_WARNING("Couldn't load robot spawn weapon '%s' in AssignWeaponPage...", weaponpage->robot_spawn_name);
  ...
  if (stricmp(weaponpage->smoke_image_name, "INVALID NAME") && weaponpage->smoke_image_name[0] != 0) {
    ...
    if (img_handle < 0) {
      LOG_WARNING("Couldn't load smoke trail file '%s' in AssignWeaponPage...", weaponpage->smoke_image_name);
  ...
  if (stricmp(weaponpage->scorch_image_name, "INVALID NAME") && weaponpage->scorch_image_name[0] != 0) {
    ...
    if (img_handle < 0) {
      LOG_WARNING("Couldn't load scorch file '%s' in AssignWeaponPage...", weaponpage->scorch_image_name);
  ...
  if (stricmp(weaponpage->icon_name, "INVALID NAME") && weaponpage->icon_name[0] != 0) {
    ...
    if (img_handle < 0) {
      LOG_WARNING("Couldn't load icon file '%s' in AssignWeaponPage...", weaponpage->icon_name);
  ...
  for (i = 0; i < MAX_WEAPON_SOUNDS; i++) {
    if (stricmp(weaponpage->sound_name[i], "INVALID NAME") && (strlen(weaponpage->sound_name[i]) > 3)) {
      ...
      if (sound_handle < 0) {
        LOG_WARNING("Couldn't load sound file '%s' in AssignWeaponPage. Weapon=%s", weaponpage->sound_name[i],
...
void mng_LoadNetWeaponPage(CFILE *infile, bool overlay) {
  ...
  if (mng_ReadNewWeaponPage(infile, &weaponpage)) {
    ...
    if (n != -1) {
      if (overlay) {
        LOG_DEBUG("OVERLAYING WEAPON %s", weaponpage.weapon_struct.name);
    ...
    LOG_WARNING("Could not load weaponpage named %s!", weaponpage.weapon_struct.name);
}
      ...
      if (Loading_addon_table != -1) {
        ...
        for (tidx = 0; tidx < addon->Num_addon_tracklocks; tidx++) {
          if (addon->Addon_tracklocks[tidx].pagetype == PAGETYPE_WEAPON &&
            ...
            // found it!!
            LOG_DEBUG("WeaponPage: %s previously loaded", weaponpage.weapon_struct.name);
