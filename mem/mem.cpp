...
void *mem_malloc_sub(int size, const char *file, int line) {
  ...
  if (!new_mem) {
    LOG_ERROR("Out of memory allocating %d bytes: line %d in %s", size, line, file);
    Int3();
    return nullptr;
  }
...
void mem_error_msg(const char *file, int line, int size) {
  LOG_ERROR("Memory error (size=%d) line %d in %s", size, line, file);
  Int3();
}
...
char *mem_strdup_sub(const char *string, const char *file, int line) {
  ...
  if (!ret) {
    LOG_ERROR("Out of memory allocating %d bytes: line %d in %s", strlen(string) + 1, line, file);
    Int3();
    return nullptr;
  }
  return ret;
...
void mem_Init() {
  ...
  if (!Heap) {
    LOG_ERROR("Unable to create memory heap! error: %d", GetLastError());
    Error("Unable to create memory heap; your system may not have enough memory to run.");
  }
  ASSERT(Heap);
  HeapCompact(Heap, 0);
  LOG_DEBUG("System Memory Status:");
  LOG_DEBUG("Percent of memory in use: %d", ms.dwMemoryLoad);
  LOG_DEBUG("Bytes of physical memory : %d", ms.dwTotalPhys);
  LOG_DEBUG("Free physical memory bytes  : %d", ms.dwAvailPhys);
  LOG_DEBUG("Available virtual memory : %d", ms.dwAvailPageFile);
...
void *mem_malloc_sub(int size, const char *file, int line) {
  ...
  if (size < 0) {
    ...
    LOG_ERROR("Offending file: %s line: %d", file, line);
    Int3();
    return NULL;
  } else if (size == 0) {
    LOG_ERROR("Warning: Zero byte malloc in %s line %d!", file, line);
    Int3();
...
void mem_shutdown() {
  ...
  free(hashTable);
  if (Total_mem_used)
    LOG_WARNING("%d bytes leaked in mem_malloc heap!", Total_mem_used);
#ifdef MEM_LOGFILE
  fclose(mem_out);
#endif
  ...
  for (int i = 0; i < MEM_MAX_MALLOCS; i++) {
    if (mem_info[i].ptr != (void *)MEM_NO_MEMORY_PTR) {
      LOG_WARNING("Memory leaked from %s line %d length %d.", mem_info[i].file, mem_info[i].line, mem_info[i].len);
      /*
      //mprintf(0,"%d\n",strdup_malloc_line);
  ...
  LOG_DEBUG("Memory library high water mark: %d", Mem_high_water_mark);
  Mem_next_slot = 0;
#endif
  if (Heap) {
    HeapDestroy(Heap);
...
void mem_heapcheck(void) {
  ...
  for (int i = 0; i < MEM_MAX_MALLOCS; i++) {
    ...
    if (memcmp((char *)freemem->ptr + freemem->len, &mem_sig, 2) != 0) {
      ...
      LOG_ERROR("Originally allocated from file %s, line %d", freemem->file, freemem->line);
      Int3();
    }
  }
#endif
